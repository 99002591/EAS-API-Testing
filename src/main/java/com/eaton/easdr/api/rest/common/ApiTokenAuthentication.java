...
import com.eaton.easdr.api.framework.ConfigFileReader;
...
public class ApiTokenAuthentication {
	...
	private static String generateToken() {
		...
		loginRequest.put("serviceAccountId", SecurityAccountToken.getServiceAcctID());
		loginRequest.put("secret", SecurityAccountToken.getSecret());
		...
		try {
			...
			authTokenValue = given().accept("application/json").contentType("application/json")
					...
					.body(loginRequest.toString()).when().post(APIObj.API_P1_SECURITY).then();
		} catch (Exception e) {
